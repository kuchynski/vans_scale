    ...
    max_height(h), sm_height(h), visiable(false), timer_id(0), enabled_ns(ens)
{
    ...
    label_sb[1]->setText(QString::fromLocal8Bit("А"));
    label_sb[2]->setText(QString::fromLocal8Bit("Б"));
    label_sb[3]->setText(QString::fromLocal8Bit("В"));

    if(enabled_ns)
    {
        spin_box_koof[3]->SetMinMax(1, 99);
        QObject::connect(spin_box_koof[3], SIGNAL(NewValue(int)), this, SLOT(OnKoofDChanged(int)));
        label_sb[4]->setText(QString::fromLocal8Bit("Д"));
        ...
        button_ns->AddButton(QString(""), image1, image2);//QString::fromLocal8Bit("ПБ") + QString::number(i+1)
