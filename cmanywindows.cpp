...
CManyWindows::CManyWindows(QWidget *parent) :// buttons(this),
    QWidget(parent), tek(0), new_tek(0), timer_id(0), mouse_press(false)
{
   ...
   // box_layout->addWidget(&buttons);
    buttons.setGeometry(QRect(0,0,100,50));
...
void CManyWindows::SetInitStruct(const SAllWindowsInit &set_struct)
{
...
void CManyWindows::AddWidget(QWidget *w, QString name, QPixmap &pix)
{
    AddWidget(w, name);
  //  buttons[buttons.size()-1]->setIcon(pix);
}
void CManyWindows::AddWidget(QWidget *w, QString name)
{
    w->setVisible(false);
...
void CManyWindows::RePlaceWidget(unsigned n, QWidget *w, QString name, QPixmap &pix)
{
...
void CManyWindows::paintEvent(QPaintEvent *p)
{
    QWidget::paintEvent(p);
}
void CManyWindows::resizeEvent(QResizeEvent *r)
{
    QWidget::resizeEvent(r);
    for(int i = 0; i < list_windows.count(); i ++)
        list_windows[i]->resize(main_widget->size());
}
void CManyWindows::OnButtonsClick(const unsigned n, bool s)
{
    if(n >= list_windows.size())
        return;
    if(timer_id)
...
void CManyWindows::timerEvent(QTimerEvent *event)
{
    timer_st --;
...
void CManyWindows::ChoiceStart()
{
    buttons.setGeometry(QRect(0,0,100,50));
    buttons.setVisible(true);
    buttons.show();
}
void CManyWindows::ChoiceEnd()
{
}
void CManyWindows::mousePressEvent(QMouseEvent *pe)
{
...
void CManyWindows::mouseMoveEvent(QMouseEvent *pe)
{
    if(mouse_press)
    {
...
void CManyWindows::mouseReleaseEvent(QMouseEvent *pe)
{
    mouse_press = false;
}
